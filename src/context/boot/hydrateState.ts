...
export type HydrateDeps = {
  normalizePriorityList: (list: unknown) => any;
  defaultLeagueRecords: () => any;
  clampFatigue: (n: number) => number;
  FATIGUE_DEFAULT: number;
  migratePracticePlan: (plan: unknown) => any;
  DEFAULT_PRACTICE_PLAN: any;
  DEFAULT_DETERMINISTIC_COUNTERS: any;
  CURRENT_SAVE_VERSION: number;
  buildMigrationEvents: (state: GameState) => any[];
  replayPersonnelOverrides: (personnelTeamOverrides: Record<string, string>, staffContractsByPersonId: Record<string, any>) => void;
  getUserTeamId: (state: GameState) => string | undefined;
};

export function hydrateState(initial: GameState, migrated: Partial<GameState>, deps: HydrateDeps): GameState {
  let out: GameState = {
    ...initial,
    ...migrated,
    ...
    saveVersion: deps.CURRENT_SAVE_VERSION,
    ...
    deterministicCounters: {
      ...deps.DEFAULT_DETERMINISTIC_COUNTERS,
