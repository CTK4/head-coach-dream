...
export function StandingsTable() {
  ...
  const renderRows = (rows: TeamStanding[]) => rows.map((row, idx) => (
    <TableRow key={row.teamId} className={`${row.teamId === userTeamId ? "border-l-4 border-l-primary bg-secondary/30" : ""}`}>
      <TableCell>{idx + 1}</TableCell><TableCell>{row.teamName}</TableCell><TableCell>{row.wins}</TableCell><TableCell>{row.losses}</TableCell><TableCell>{row.ties}</TableCell><TableCell>{row.winPct.toFixed(3)}</TableCell><TableCell>{row.pointsFor}</TableCell><TableCell>{row.pointsAgainst}</TableCell><TableCell>{row.streak}</TableCell>
    </TableRow>
  ));

  return <div className="space-y-3"><Tabs value={view} onValueChange={(v) => setView(v as View)}><TabsList><TabsTrigger value="DIV">Division</TabsTrigger><TabsTrigger value="CONF">Conference</TabsTrigger><TabsTrigger value="LEAGUE">League</TabsTrigger></TabsList></Tabs><Table><TableHeader><TableRow><TableHead>Rank</TableHead><TableHead>Team</TableHead><TableHead>W</TableHead><TableHead>L</TableHead><TableHead>T</TableHead><TableHead>PCT</TableHead><TableHead>PF</TableHead><TableHead>PA</TableHead><TableHead>Streak</TableHead></TableRow></TableHeader><TableBody>{view === "LEAGUE" ? renderRows(standings) : null}{view === "CONF" ? Array.from(grouped.entries()).flatMap(([conf, rows]) => [<TableRow key={`h-${conf}`}><TableCell colSpan={9} className="font-semibold">{conf}</TableCell></TableRow>, ...renderRows(rows)]) : null}{view === "DIV" ? Array.from(grouped.entries()).flatMap(([conf, rows]) => { const byDiv = new Map<string, typeof rows>(); for (const r of rows) { const list = byDiv.get(r.division) ?? []; list.push(r); byDiv.set(r.division, list); } return [<TableRow key={`c-${conf}`}><TableCell colSpan={9} className="font-semibold">{conf}</TableCell></TableRow>, ...Array.from(byDiv.entries()).flatMap(([div, divRows]) => [<TableRow key={div}><TableCell colSpan={9} className="text-muted-foreground">{div}</TableCell></TableRow>, ...renderRows(divRows)])]; }) : null}</TableBody></Table></div>;
}
