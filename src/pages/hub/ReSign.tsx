...
export default function ReSignPage() {
  ...
  function offer() {
    if (!selected) return;
    const accepted = offeredApy >= expectedApy;
    const reason = accepted ? "Extension accepted." : "Extension rejected. Increase salary to reach expected market value.";
    setResult({ accepted, reason });
    setResultOpen(true);
    if (!accepted) return;
    dispatch({
      type: "EXTEND_PLAYER",
      payload: { playerId: selected.playerId, years, apy: offeredApy, signingBonus, guaranteedAtSigning },
    } as any);
  }
  ...
  return (
    <div className="space-y-4 overflow-x-hidden">
      ...
      <Dialog open={resultOpen} onOpenChange={setResultOpen}><DialogContent className="border-slate-300/15 bg-slate-950 text-slate-100"><DialogHeader><DialogTitle>{result?.accepted ? "Extension Accepted" : "Extension Rejected"}</DialogTitle><DialogDescription className="text-slate-200/70">{result?.reason ?? ""}</DialogDescription></DialogHeader><Button onClick={() => setResultOpen(false)}>Close</Button></DialogContent></Dialog>
    </div>
  );
}
