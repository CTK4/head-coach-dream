...
const POSITION_FILTERS = ["ALL", "QB", "RB", "WR", "TE", "OL", "DT", "EDGE", "LB", "CB", "S"];
  ...
  return (
    <div className="space-y-3 p-4">
      <div className="flex items-center justify-between">
        ...
        <div>{onClock ? "YOU ARE ON THE CLOCK" : "CPU SIMULATING"}</div>
      </div>

      <div className="overflow-x-auto pb-1">
        <div className="flex min-w-max gap-2">
          {POSITION_FILTERS.map((x) => (
    ...
    <div className="p-4 space-y-3">
      ...
      {available.slice(0, 80).map((p) => (
        <div key={p.prospectId} className="flex items-center justify-between gap-3 rounded border p-3">
          <div>
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex flex-col gap-2">
            <button
              ...
              onClick={() => dispatch({ type: "DRAFT_USER_PICK", payload: { prospectId: p.prospectId } })}
            >
              Draft Player
            </button>
            <button
