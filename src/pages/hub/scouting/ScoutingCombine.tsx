      ...
      {activeTab === "SHORTLIST" ? (
        <div className="space-y-2">
          {shortlist.map((prospect) => {
            ...
            const alreadyInterviewedToday = interviewedToday.includes(prospect.id);
            const interviewDisabled = !interviewsEnabled || interviewsRemaining <= 0 || alreadyInterviewedToday;
              ...
              <div key={prospect.id} className="flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/5 p-3">
                <div className="min-w-0">
                  <div className="truncate font-semibold">
                    <button type="button" className="text-sky-300 hover:underline" onClick={() => openProspectProfile(prospect.id)}>
                      {prospect.draft.name}
                    </button>{" "}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    type="button"
                    className={`rounded border px-3 py-1 text-xs ${interviewDisabled ? "cursor-not-allowed border-white/10 text-white/40" : "border-amber-500 text-amber-200"}`}
                    disabled={interviewDisabled}
                    onClick={() => dispatch({ type: "SCOUT_COMBINE_INTERVIEW", payload: { prospectId: prospect.id, category: (prospect.profile.clarity.FIT ?? 0) <= (prospect.profile.clarity.CHAR ?? 0) ? "IQ" : "LEADERSHIP" } })}
                  >
                    {alreadyInterviewedToday ? "INTERVIEWED TODAY" : !interviewsEnabled ? "DAY 4 ONLY" : "ADD INTERVIEW"}
                  </button>
                </div>
