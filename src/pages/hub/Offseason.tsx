...
const STEPS_OPEN_WITHOUT_COORDINATORS = new Set(["RESIGNING", "COMBINE", "PRE_DRAFT"]);
      ...
      <Card>
        <CardContent className="p-6 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={goCurrent} disabled={currentStepBlocked}>
              Open Current Step
            </Button>
      ...
      <Card>
        <CardContent className="p-5 space-y-2">
          ...
          {OFFSEASON_STEPS.map((s, idx) => {
            ...
            const done = !!state.offseason.stepsComplete[s.id];
            const isBlocked = !ready && !STEPS_OPEN_WITHOUT_COORDINATORS.has(s.id);

            return (
              <div
                key={s.id}
                className={`flex items-center justify-between rounded-md border px-3 py-2 ${isCurrent ? "bg-secondary/50" : ""}`}
              >
                ...
                <div className="flex items-center gap-2">
                  <Badge variant={done ? "default" : isCurrent ? "secondary" : "outline"}>{done ? "Done" : isCurrent ? "Now" : "Later"}</Badge>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => navigate(stepRoute[s.id] ?? "/offseason/free-agency")}
                    disabled={isBlocked}
                  >
                    Open
                  </Button>
