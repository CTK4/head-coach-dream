...
export function resolveMoveAdvantage(input: MoveInput): number {
  ...
  const angleVariance = Math.min(90, Math.max(0, Math.abs(input.angleDeg))) / 90;
  ...
  if (input.moveType === "STIFF_ARM") {
    return handStrength(ratingZ(input.ballcarrier.strength)) * 0.08 + reachAdv * 0.014 + (Math.abs(input.angleDeg) > 20 ? 0.08 : 0);
  }
  return 0;
}
